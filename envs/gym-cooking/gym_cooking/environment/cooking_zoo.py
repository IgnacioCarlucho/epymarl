...
class CookingEnvironment(Env):
    ...
    def observation_space(self):
        ...
        # print()
        # print( feature_vec_nc_len, self.possible_agents)
        # print(self.world.width, self.world.height, self.graph_representation_length, feature_vec_len)
        # print()
        # print(obs_space_dict[self.obs_spaces[0]])

        

        min_obs = [-1.]*feature_vec_nc_len
        max_obs = [1.]*feature_vec_nc_len
        obs_space = gym.spaces.Box(np.array(min_obs), np.array(max_obs), dtype=np.float32)
        obs_space_dict_ =  gym.spaces.Tuple(tuple([obs_space] * self.n_agents))


        return obs_space_dict_ # obs_space_dict[self.obs_spaces[0]]
    ...
    def step(self, action):
        ...
        return self.observations, [np.sum(total_rewards)]*self.n_agents, [all_done]*self.n_agents, {}
