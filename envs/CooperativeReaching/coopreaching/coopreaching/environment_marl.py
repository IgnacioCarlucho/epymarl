...
class MARLCooperativeReachingEnv(Env):
    ...
    def __init__(
        ...
        # self.new_observation_space = self._get_observation_space_real()
        # # Changing observation space 
        # self.observation_space = self.old_observation_space
        # # print("self.old_observation_space", self.old_observation_space)
    ...
    def observation_space(self):
        ...
        return self._get_observation_space_new()

    @property
    def new_observation_space(self):
        return self._get_observation_space_real()
    ...
    def _get_observation_space_new(self):
        """The Observation Space for each agent.
        - all of the board (board_size^2) with foods
        - player description (x, y)*player_count
        """
        min_obs = [0, 0, 0, 0] # * len(self.players)
        max_obs = [self.world_length-1, self.world_height-1, self.world_length-1, self.world_height-1] # * len(self.players)

        gymbox = gym.spaces.Box(np.array(min_obs), np.array(max_obs), dtype=np.float32)

        return gym.spaces.Tuple(tuple([gymbox] * len(self.players)))
    ...
    def reset(self, seed=None):
        ...
        #print("np.array([a[1] for a in nobs])", np.array([a[1] for a in nobs]))
        #print("np.array([a[1] for a in nobs])", nobs)
        #obs_ = np.array([a[1] for a in nobs])
        return np.array([a[1] for a in nobs])
    ...
    def step(self, action):
        ...
        return np.asarray([a[1] for a in nobs]), [nreward[0]]*2, [ndone]*2, ninfo[0]
